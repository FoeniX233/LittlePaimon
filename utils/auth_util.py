    ...
    if not cookies:
        if cache_cookie:
            if cache_cookie['type'] == 'public':
                logger.info(f'---打工战士调用{uid}的缓存公共cookie执行{action}操作---')
            else:
                logger.info(f'---打工战士调用{uid}的缓存私人cookie执行{action}操作---')
            return cache_cookie
        public_cookie = await get_public_cookie()
        if not public_cookie:
            logger.info(f'---打工战士当前没有可用的公共cookie，可能是都达到了上限或没有公共cookie---')
            return None
        else:
            logger.info(f'---打工战士调用{public_cookie[0]}号公共cookie执行{action}操作---')
        ...
        for user_id_, cookie, uid_, mys_id_ in cookies:
            if (uid and uid_ == uid) or (mys_id and mys_id_ == mys_id):
                logger.info(f'---打工战士调用用户{user_id_}的uid{uid_}私人cookie执行{action}操作---')
        ...
        logger.info(f'---打工战士调用用户{use_cookie[0]}的uid{use_cookie[2]}私人cookie执行{action}操作---')
...
async def get_own_cookie(uid='', mys_id='', action=''):
    ...
    if not cookie:
        ...
        cookie = cookie[0]
        logger.info(f'---打工战士调用用户{cookie[0]}的uid{cookie[2]}私人cookie执行{action}操作---')
