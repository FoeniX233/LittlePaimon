...
async def addFriend(bot: Bot, event: FriendRequestEvent):
    superuser_msg = f'{event.user_id}请求添加打工战士为好友, 验证信息为：{event.comment}'
...
async def addGroup(bot: Bot, event: GroupRequestEvent):
    if event.sub_type != 'invite':
        return
    superuser_msg = f'{event.user_id}邀请打工战士加入群{event.group_id}'
...
async def FriendNew(bot: Bot, event: FriendAddNoticeEvent):
    greet_emoticon = MessageBuild.Image(Path() / 'resources' / 'LittlePaimon' / 'emoticons' / '打工战士-干杯.png', mode='RGBA')
    await sleep(random.randint(4, 8))
...
async def GroupTalkative(bot: Bot, event: HonorNotifyEvent):
    if event.group_id not in config.paimon_greet_ban and event.honor_type == 'talkative':
        await sleep(random.randint(4, 8))
        if event.user_id == event.self_id:
            honor_emoticon = MessageBuild.Image(Path() / 'resources' / 'LittlePaimon' / 'emoticons' / '打工战士-哼哼.png',
                                                mode='RGBA')
            text = random.choice(['诶嘿~本应急食品是龙王~~', '哦豁，打工战士又是龙王，你们好逊哦(', '怎么回事，你们这么多人居然说不过我一个应急食品?~', '请叫我龙王打工战士~诶嘿'])
            await bot.send_group_msg(group_id=event.group_id,
                                     message=Message(MessageSegment.text(text) + honor_emoticon))
        elif random.random() <= 0.5:
            honor2_emoticon = MessageBuild.Image(Path() / 'resources' / 'LittlePaimon' / 'emoticons' / '打工战士-黑线.png',
                                                 mode='RGBA')
            text = random.choice(['怎么这人比我打工战士话还多!!', '咦~是个话唠龙王(', '好气哦，怎么能抢我打工战士的龙王啊!!'])
            await bot.send_group_msg(group_id=event.group_id,
