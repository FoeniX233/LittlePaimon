...
def create_choice_command(endswith: str, type_: str, url: str, tips: str = None, help_tips: str = None):
    ...
    async def _(event: MessageEvent, state: T_State):
        ...
        if choice not in match_alias:
            ...
            if state['times'] == 1:
                await command.reject(f'请旅行者从上面的{endswith[0:2]}中选一个问打工战士\n回答\"q\"可以取消查询', at_sender=True)
            elif state['times'] == 2:
                await command.reject(f'别调戏打工战士啦，快选一个吧，不想问了请回答\"q\"！', at_sender=True)
            elif state['times'] >= 3:
                await command.finish(f'看来旅行者您有点神志不清哦(，下次再问打工战士吧' + MessageSegment.face(146), at_sender=True)
