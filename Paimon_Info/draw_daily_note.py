...
async def draw_daily_note_card(data, uid):
    ...
    if data['current_resin'] == 160:
        bg_draw.text((200, 220), f"树脂满了哦~", fill='#7b8386', font=get_font(12, 'HYWenHei-55W.ttf'))
    else:
        ...
        bg_draw.text((200, 220), recover_time_str, fill='#7b8386', font=get_font(12, 'HYWenHei-55W.ttf'))

    # 委托文字
    bg_draw.text((200, 260), f"{data['finished_task_num']}/4", fill='#7b8386',
                 font=get_font(30, 'HYWenHei-85W.ttf'))
    if data['finished_task_num'] == 4:
        bg_draw.text((200, 295), "今日委托已全部完成~", fill='#7b8386', font=get_font(12, 'HYWenHei-55W.ttf'))
    else:
        bg_draw.text((200, 290), "今日委托完成情况", fill='#7b8386', font=get_font(12, 'HYWenHei-55W.ttf'))

    # 周本文字
    bg_draw.text((200, 335), f"{3 - data['remain_resin_discount_num']}/3",fill='#7b8386',
                 font=get_font(30, 'HYWenHei-85W.ttf'))
    if data['remain_resin_discount_num'] == 0:
        bg_draw.text((200, 370), "周本已完成", fill='#7b8386', font=get_font(12, 'HYWenHei-55W.ttf'))
    else:
        bg_draw.text((200, 370), f"剩余{data['remain_resin_discount_num']}次周本减半", fill='#7b8386',
    ...
    if data['transformer']['obtained']:
        ...
        if rt['Day'] == 0 and rt['reached']:
            ...
            bg_draw.text((200, 450), f"{rt['Hour']}小时后可用", fill='#7b8386', font=get_font(12, 'HYWenHei-55W.ttf'))
        else:
            bg_draw.text((200, 415), "冷却中", fill='#7b8386', font=get_font(30, 'HYWenHei-85W.ttf'))
            bg_draw.text((200, 450), f"{rt['Day']}天后", fill='#7b8386', font=get_font(12, 'HYWenHei-55W.ttf'))
    else:
    ...
    if exp:
        ...
        for role in exp:
            ...
            bg_img.alpha_composite(role_avatar, (543, 73 * i + 127))
            if role['status'] == 'Ongoing':
                hour = int(role['remained_time']) // 3600

                minute = int(role['remained_time']) % 3600 // 60
                bg_draw.text((615, 73 * i + 143), f"{hour}:{minute}完成", fill='#7b8386',
                             font=get_font(20, 'HYWenHei-85W.ttf'))
            else:
                bg_draw.text((615, 73 * i + 143), "已完成", fill='#7b8386',
                             font=get_font(20, 'HYWenHei-85W.ttf'))
            i += 1
        ...
        if max_time == 0:
            ...
            bg_draw.text((614, 90), f"最快将于{last_finish_str}完成", fill='#7b8386',
                             font=get_font(20, 'HYWenHei-85W.ttf'))
    else:
        bg_draw.text((640, 90), "没有进行探索派遣", fill='#7b8386',
                             font=get_font(20, 'HYWenHei-85W.ttf'))
